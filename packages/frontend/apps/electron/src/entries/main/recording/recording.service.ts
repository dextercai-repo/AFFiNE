...
export class RecordingManager implements OnModuleInit {
  ...
  shareableContent: ShareableContent | null = null;
  ...
  private readonly createAppGroup = (
    ...
    const groupProcess =
      this.shareableContent?.applicationWithProcessId(processGroupId);
    if (!groupProcess) {
      return;
    }
  ...
  private readonly setupMediaListeners = () => {
    const SCClass: typeof ShareableContent =
    ...
    this.subscribers.push(
      ...
      SCClass.onApplicationListChanged(() => {
        this.updateApplicationsPing$.next(Date.now());
      }),
        ...
        apps.forEach(app => {
          try {
            ...
            _appStateSubscribers.push(
              SCClass.onAppStateChanged(tappableApp, () => {
                this.updateApplicationsPing$.next(Date.now());
              })
            );
          } catch (error) {
  ...
  startRecording(appGroup?: AppGroupInfo | number): RecordingStatus | null {
    ...
    setTimeout(() => {
      ...
      if (
        ...
        state.id === this.recordingStatus$.value?.id
      ) {
        this.stopRecording(state.id).catch(err => {
          this.logger.error('failed to stop recording', err, this.context);
        });
      }
  ...
  getAllApps(): TappableAppInfo[] {
    ...
    const apps = this.shareableContent.applications().map(app => {
      try {
        return {
          rawInstance: app,
